  ...
  const confirmStatusChange = async () => {
    ...
    if (pendingUpdate) {
      ...
      if (typeof statusId === 'string') {
        ...
        if (idx !== -1) {
          statusId = idx + 1; // e.g. New Task -> 1, Open for review -> 2, etc.
        }
      }
      ...
      try {
        ...
        let taskData = fullTaskRes?.data || fullTaskRes;
        ...
        if (taskData) {
          const payload = {
            taskId: realTaskId,
            taskUid: taskData.taskUid || taskData.title || '',
            categoryId: parseInt(taskData.categoryId || taskData.category, 10) || 0,
            subCategoryId: parseInt(taskData.subCategoryId || taskData.subCategory, 10) || 0,
            assignBy: parseInt(taskData.assignBy || taskData.assignedBy, 10) || 0,
            reviewTo: parseInt(taskData.reviewTo, 10) || 0,
            priority: parseInt(taskData.priority, 10) || 0,
            dueDate: taskData.dueDate,
            status: parseInt(statusId, 10) || 0,
            taskDesc: taskData.taskDesc || taskData.description || taskData.taskUid || taskData.title || '',
            notes: taskData.notes || ''
          };

          await taskService.updateTask(realTaskId, payload);
        }
                  ...
                  <td className={cn("px-6 py-5 font-medium", isDarkMode ? "text-slate-300" : "text-slate-600")}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold shadow-sm overflow-hidden">
                        {(task.assignByName || task.assignedTo || 'U').substring(0, 2).toUpperCase()}
                      </div>
                      {task.assignByName || task.assignedTo || 'Unassigned'}
                    </div>
                  </td>
                  <td className="px-6 py-5">
