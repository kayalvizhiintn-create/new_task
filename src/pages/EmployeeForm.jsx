...
import { ArrowLeft, Save, User, Shield, MapPin, Key, RefreshCw } from 'lucide-react';
...
export default function EmployeeForm() {
  ...
  const { isDarkMode, places, userPrivileges, currentUser, permissionCodes = {} } = useStore();

  const isEditMode = Boolean(id);
  const [loading, setLoading] = React.useState(isEditMode);
  const [submitting, setSubmitting] = React.useState(false);
  const [apiRoles, setApiRoles] = React.useState([]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const errMsg = error.response?.data?.message || error.response?.data?.Message || "Failed to save employee.";
      Swal.fire({
  ...
  const { check, isReadOnly } = usePermission();

  const _addEmpPerm = permissionCodes['addemployee.page'] ?? 'Deny';
  const _empEditPerm = permissionCodes['employee.edit'] ?? 'Deny';
  const canAccess = isEditMode
    ? (_empEditPerm !== 'Deny' && _empEditPerm !== 'Hidden')
    : (_addEmpPerm !== 'Deny' && _addEmpPerm !== 'Hidden');
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8" autoComplete="off">
          ...
          <div className={cn("p-8 rounded-3xl border shadow-sm transition-all duration-300", isDarkMode ? "bg-slate-800/40 border-slate-700/50" : "bg-white border-slate-200")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <SecureComponent code="addemployee.fullname">
                <div>
                  <label className={labelClasses}>Full Name <span className="text-rose-500">*</span></label>
                  <input
                    {...register('name', { required: 'Name is required' })}
                    disabled={isReadOnly('addemployee.fullname')}
                    className={cn(inputClasses, errors.name && "border-rose-500 focus:border-rose-500 focus:ring-rose-500/20", isReadOnly('addemployee.fullname') && "opacity-60 cursor-not-allowed")}
              ...
              <SecureComponent code="addemployee.email">
                <div>
                  ...
                  <input
                    ...
                    disabled={isReadOnly('addemployee.email')}
                    className={cn(inputClasses, errors.email && "border-rose-500 focus:border-rose-500 focus:ring-rose-500/20", isReadOnly('addemployee.email') && "opacity-60 cursor-not-allowed")}
              ...
              <SecureComponent code="addemployee.profile">
                <div className="md:col-span-2">
                  ...
                  {avatarImage ? (
                    <div className="flex items-center gap-6 mt-2">
                      ...
                      {!isReadOnly('addemployee.profile') && (
                        <button
                          type="button"
                    ...
                    <>
                      <input
                        ...
                        disabled={isReadOnly('addemployee.profile')}
                        className={cn(inputClasses, "file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer", isReadOnly('addemployee.profile') && "opacity-60 cursor-not-allowed")}
                      />
          ...
          <div className={cn("p-8 rounded-3xl border shadow-sm transition-all duration-300", isDarkMode ? "bg-slate-800/40 border-slate-700/50" : "bg-white border-slate-200")}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <SecureComponent code="addemployee.bioid">
                <div>
                  <label className={labelClasses}>Bio ID <span className="text-rose-500">*</span></label>
                  <input
                    {...register('bioId', { required: 'Bio ID is required' })}
                    disabled={isReadOnly('addemployee.bioid')}
                    className={cn(inputClasses, errors.bioId && "border-rose-500", isReadOnly('addemployee.bioid') && "opacity-60 cursor-not-allowed")}
              ...
              {!isEditMode && (
                <SecureComponent code="addemployee.password">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        disabled={isReadOnly('addemployee.password')}
                        className={cn(inputClasses, errors.password && "border-rose-500", isReadOnly('addemployee.password') && "opacity-60 cursor-not-allowed")}
              ...
              <SecureComponent code="addemployee.role">
                <div>
                  <label className={labelClasses}>Role <span className="text-rose-500">*</span></label>
                  <div className={cn(isReadOnly('addemployee.role') && "pointer-events-none opacity-60")}>
                    <SearchableSelect
                      {...register('role', { required: 'Role is required' })}
                      options={apiRoles.map(r => ({
                        value: String(r.id || r._id || r.roleId || r.value),
                        label: r.name || r.roleName || r.title || r.value
                      }))}
              ...
              <SecureComponent code="addemployee.location">
                <div>
                  <label className={labelClasses}>Work Location (Place) <span className="text-rose-500">*</span></label>
                  <div className={cn("relative", isReadOnly('addemployee.location') && "pointer-events-none opacity-60")}>
                    <SearchableSelect
                    ...
                    />
                    <MapPin className="w-5 h-5 absolute right-12 top-3.5 text-slate-400 pointer-events-none" />
                  </div>
                  {errors.place && <p className="text-rose-500 text-sm font-semibold mt-2">{errors.place.message}</p>}
                </div>
              </SecureComponent>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4">
            <SecureComponent code="addemployee.cancel">
              <Link to="/employees" className={cn("px-6 py-3 font-bold rounded-xl transition-all duration-300", isDarkMode ? "bg-slate-800 text-slate-300 hover:bg-slate-700" : "bg-white border text-slate-600 hover:bg-slate-50")}>
                Cancel
              </Link>
            </SecureComponent>
            <SecureComponent code="addemployee.submit">
              <button
                type="submit"
                disabled={submitting}
                className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all duration-300 shadow flex items-center gap-2 disabled:opacity-70"
              >
