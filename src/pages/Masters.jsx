...
import { Settings, FolderTree, Shield, MapPin, AlertTriangle, Activity, Plus, Trash2, ChevronRight, BarChart2, Edit2, Check, X, Users } from 'lucide-react';
...
const getItemName = (item) => {
  ...
  return item.name || item.title || item.value || item.categoryName || item.subcategoryName || item.subCategoryName || item.taskDesc || item.roleName || item.priorityName || item.statusName || item.empName || item.employeeName || 'Unknown';
};
...
export default function Masters() {
  const { isDarkMode, places, dashboardMetrics, addMasterItem, editMasterItem, removeMasterItem } = useStore(); 
