...
export default function ProjectFlow({ task, onClose, isInline = false }) {
  ...
  const content = (
    <div 
      ref={isInline ? null : modalRef}
      className={cn(
        "w-full flex flex-col overflow-hidden",
        isInline 
          ? "rounded-3xl border h-[420px]" 
          : "max-w-2xl max-h-[90vh] rounded-3xl shadow-2xl animate-[scaleIn_0.3s_ease-out]",
        isDarkMode 
          ? (isInline ? "bg-slate-800/40 border-slate-700/50" : "bg-slate-800 border border-slate-700") 
          : (isInline ? "bg-white/50 border-slate-200" : "bg-white border border-slate-200")
      )}
    >
      {/* Header */}
      <div className={cn("px-6 py-4 flex items-center justify-between border-b", isDarkMode ? "border-slate-700" : "border-slate-100")}>
        <div>
          ...
          <p className={cn("text-sm font-medium", isDarkMode ? "text-slate-400" : "text-slate-500")}>UID: {task.taskUid || `TSK-${String(task.taskId || task.id).padStart(3, '0')}`} - {task.title || task.taskDesc}</p>
        </div>
        {!isInline && onClose && (
          <button 
            onClick={onClose}
            ...
            {history.map((entry, index) => (
              <div key={index} className="relative pl-8 md:pl-10">
                ...
                <div className={cn("p-4 rounded-2xl border shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5", 
                  ...
                  <p className={cn("text-sm font-semibold mb-2 whitespace-pre-wrap", isDarkMode ? "text-slate-200" : "text-slate-800")}>
                    {entry.details}
                  </p>
                  <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full bg-slate-250 dark:bg-slate-700 flex items-center justify-center text-[10px] font-bold">
